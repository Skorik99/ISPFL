...
class GPR(torch.nn.Module):
    ...
    def Select_Clients(
        ...
        weights_tensor = (
            None
            if weights is None
            else torch.as_tensor(weights, device=self.device, dtype=torch.float)
        )

        def max_loss_decrease_client(client_group, Sigma):
            ...
            if weights_tensor is None:
                ...
                total_loss_decrease = (
                    torch.sum(
                        weights_tensor[client_group].reshape([len(client_group), 1])
                        * Sigma_valid,
                        dim=0,
                    )
                    * Diag_valid
