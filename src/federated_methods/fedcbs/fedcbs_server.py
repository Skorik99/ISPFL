...
class FedCBSServer(FedAvgServer):
    def __init__(self, cfg, lambda_, df):
        super().__init__(cfg)
    ...
    def get_clients_data_info(self):
        client_distr = {}
        client_data_count = {}
        unique_classes = sorted(self.df["target"].unique())
        amount_classes = len(unique_classes)

        for num_client in range(self.amount_of_clients):
            client_data = self.df[self.df["client"] == num_client]
            target_counts = (
                client_data["target"]
    ...
    def create_qcid_matrix(self):
        client_distributions = np.array(
            [
                list(self.client_distr[client].values())
                for client in range(self.amount_of_clients)
            ]
        )
        client_counts = np.array(
            [
                self.client_data_count[client]
                for client in range(self.amount_of_clients)
            ]
        )
        weighted_distributions = client_distributions * client_counts[:, None]
        qcid_mtr = weighted_distributions @ weighted_distributions.T
    ...
    def select_clients_to_train(self, num_clients_subset, server_sampling=False):
        ...
        for m in range(num_clients_subset):
            if m == 0:
                ...
                current_qcid = self.qcid_fn(selected_clients)
                probabilities = np.array(
                    [
                        1.0
                ...
                current_qcid = self.qcid_fn(selected_clients)
                candidate_qcids = np.array(
                    [
                        self.qcid_fn(selected_clients + [client])
                        for client in remaining_clients
                    ]
                )
                probabilities = np.array(
                    (current_qcid / candidate_qcids) ** betas[m - 2]
                    / candidate_qcids
                )
            ...
            selected_client = np.random.choice(remaining_clients, p=probabilities).item()
